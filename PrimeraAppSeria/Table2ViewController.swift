...
class Table2ViewController: UITableViewController {
   ...
   override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
      let cell = tableView.dequeueReusableCell(withIdentifier: "zeldaMore", for: indexPath) as! TableViewZelda
      let dato = moredata[indexPath.row]
      cell.usuario.text = dato.username
      cell.name.text = "\(dato.last_name), \(dato.first_name)"
      cell.jobtitle.text = dato.jobtitle
      cell.domicilio.text = dato.address
      cell.ciudad.text = dato.city
